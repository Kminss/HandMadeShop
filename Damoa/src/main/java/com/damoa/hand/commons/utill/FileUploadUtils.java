...
public class FileUploadUtils {
	   private static final Logger logger = LoggerFactory.getLogger(FileUploadUtils.class);
	    ...
	    public static String uploadFile(MultipartFile file, HttpServletRequest request) throws Exception {
	        ...
	        if (ImgUtils.getMediaType(originalFileName) != null) {
	    ...
	    public static void deleteFile(String fileName, HttpServletRequest request) {
	        ...
	        // 1. 원본 이미지 파일 삭제
	        MediaType mediaType = ImgUtils.getMediaType(fileName);
	        if (mediaType != null) {
	...
	/*
	 * // 썸네일 이미지 생성 private static String makeThumbnail(String uploadRootPath,
	 * String datePath, String fileName) throws Exception {
	 * 
	 * // 원본이미지를 메모리상에 로딩 BufferedImage originalImg = ImageIO.read(new
	 * File(uploadRootPath + datePath, fileName)); // 원본이미지를 축소 BufferedImage
	 * thumbnailImg = Scalr.resize(originalImg, Scalr.Method.AUTOMATIC,
	 * Scalr.Mode.FIT_TO_HEIGHT, 100); // 썸네일 파일명 String thumbnailImgName = "s_" +
	 * fileName; // 썸네일 업로드 경로 String fullPath = uploadRootPath + datePath +
	 * File.separator + thumbnailImgName; // 썸네일 파일 객체생성 File newFile = new
	 * File(fullPath); // 썸네일 파일 확장자 추출 String formatName =
	 * ImgUtils.getFormatName(fileName); // 썸네일 파일 저장 ImageIO.write(thumbnailImg,
	 * formatName, newFile);
	 * 
